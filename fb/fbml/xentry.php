...
function UpdLaddPos( $champkey, $roundnum, $SortOrd ) {	// $SortOrd not used?
  ...
  $Iter = 1;
  //$akey = array_keys( $TeamHash );
  usort( $TeamHash, "SortTeams" );
  foreach( $akey as $key ) {	// (sort //SortTeams keys %TeamHash) 
	//$LTeamRec
	...
	if( ( $LTeamRec = mysql_fetch_array( $LTeamRecQ ) ) && ($LTeamRec["PosExists"] > 0)) {
	  ...
	  $query = "UPDATE TeamLadderPos
		...
		WHERE TeamKey = ".$key."
		AND RoundNumber = ".$roundnum;
	  mysql_query( $query ) or die( '{"error" : "ScoreTank error (e42)"}' );
	} else {
	  // $LTeamRec
	  $query = "INSERT INTO TeamLadderPos (TeamKey, RoundNumber, LadderPos)
			VALUES ($key, ".$roundnum.", $Iter)";
	  mysql_query( $query ) or die( '{"error" : "ScoreTank error (e43)"}' );
	}
	$Iter++;
  }
}
...
$DataRecQ = mysql_query( $query ) or die( '{"error" : "ScoreTank error (e31)"}' );
	...
	if( $AwayRes["Result"] ) {
	  ...
	  if( $AwayRes["Result"] == "W" ) {
	    ...
	    if( $AwayRes["AwayTeamSupScore"] > $AwayRes["HomeTeamSupScore"] ) {
	      $BPoints += 2;
		} else if( $AwayRes["HomeTeamSupScore"] == $AwayRes["AwayTeamScupScore"] ) {
	      $BPoints += 1;
	    }
	  }
...
//die( '{"error" : "xFinals ' . $MRRef . '"}' );
  ...
  if( ! $ChkScoreless ) {
	// $UpdRec
	$query = "SELECT DISTINCTROW Count(*) AS NMatchLeft
			  FROM NMatch
	...
	if( $UpdRec = mysql_fetch_array( $UpdRecQ ) ) {
	  ...
	  if( false ) { // $_[2]
	    return( '{"error" : "Results are closed for these matches"}' );
	  } else {
		return( '{"error" : "Results are closed for this match (2)"}' );
	  }
	}
	$ChkScoreless = 1;
  }
...
//die( '{"error" : "Home: ' . $home . ', Away: ' . $away . '"}' );
  ...
  mysql_query( $query ) or die( '{"error" : "ScoreTank error (e4 - ' . $res . ', ' . $HTScore . ', ' . $ATScore . ', ' . $Score . ')"}' );
...
//die( '{"error" : "PLx" }' );
  ProcessLadd( $chkey, $rnum );
  ProcessDerived( $chkey );
//die( '{"error" : "PLx" }' );
