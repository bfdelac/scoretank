...
WHERE TeamKey = $TeamNum");
	$TeamRecq = mysql_query( $TeamRecq ) or die( mysql_error( ) );
...
ORDER BY RoundNumber, MatchNumber" );
  $FMatchRecq = mysql_query( $query ) or die( mysql_error( ) );
...
//$date =~ s/^0//;
  ...
  "<TR><TD colspan='6'>" . $ven . "</TD></TR>";
           ...
           //Calculate the streak
            ...
            //$Fetch = $FRec->FetchRow( );
			$FRec = mysql_fetch_array( $MatchRecq, MYSQL_ASSOC );
            if( $Finals == 1 ) {
                $LastSer = $FRec["SeriesNumber"];
            }
            if( !$FRec && ( $Finals == 0 ) ) {
                ...
                // $Fetch = $FRec->FetchRow( );
                $LastRound = 0;
                if( $FRec = mysql_fetch_array( $FMatchRecq, MYSQL_ASSOC ) ) {
